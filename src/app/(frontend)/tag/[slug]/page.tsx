...
import { PayloadPost, PayloadTag, getPostImageUrl, getPostImageAlt } from "@/types";
import { decodeHtmlEntities } from "@/utils/utils";

// Enable ISR with 1 hour revalidation for better caching
export const revalidate = 3600;

async function getTagBySlug(slug: string): Promise<PayloadTag | null> {
  ...
  return (result.docs[0] as PayloadTag) || null;
}
...
export default async function TagPage({
  ...
  const { posts, hasNextPage, hasPrevPage, totalPages } = await getPostsByTag(
    tag.id,
    currentPage,
    postsPerPage
  );
